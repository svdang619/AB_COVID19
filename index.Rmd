...
author: "Steven Dang"
date: "Last updated on 22 February 2022"
output:
...
valueBox(value = prettyNum(last(Hosp_num$Hosp), big.mark = ","), 
         caption = paste0("Hospitalizations", "<br>", "Change: ", prettyNum(last(Hosp_num$Diff), big.mark = ","), "<br>","Record High: ", prettyNum(max(Hosp_num$Hosp), big.mark = ",")),
         color = "orange",
...
valueBox(value = prettyNum(last(ICU_num$ICU_case), big.mark = ","), 
         caption = paste0("Patients in ICU", "", "Change: ", prettyNum(last(ICU_num$Diff), big.mark = ","), "<br>",
...
```{r Alberta active cases leaflet, eval = TRUE}


numeric <- as.numeric(leaflet_geojson$active_cases)
colbins <- c(0,10,50,75,100,200,400,600,800,1000)
col <- colorBin("GnBu", domain = as.numeric(leaflet_geojson$active_cases), bins = colbins)
...
### Total AB Population
```{r Total AB Population Two doses}
...
```{r Active Cases by age,  eval = TRUE}
...
```{r COVID-19 by Gender and Age, eval = TRUE}
...
```{r Active cases by zone, eval = TRUE}
...
leaflet(Vacc_poly) %>% addProviderTiles("Esri")%>% setView(lng = -115, lat = 55, zoom = 5) %>% 
  ...
  leaflet::addLegend(pal = vacc1_col_bins,
...
Historical_SHP_get@data$Labeltext <- paste0(
  " Location: ", Historical_SHP_get@data$location,
  " Cases: ", format(Historical_SHP_get@data$cases_active, nsmall = 0, big.mark = ",")
)
...
### Update
```{r World Date}
valueBox(value = "4 November 2021" , 
...
valueBox(value = prettyNum(sum(ts_confirmed_global$Confirmed_today), big.mark = ","), 
         caption = paste0("Confirmed Cases", "", "New Cases: +", World_Cases_Diff),
         icon = "fas fa-chart-bar", 
         color = "teal"
         )
```
...
valueBox(value = prettyNum(sum(ts_deaths_global$Deaths_today), big.mark = ","),
         caption = paste0("Deaths", "", "New Deaths: +", World_Deaths_Diff),
         color = "red", 
         icon = "fas fa-skull-crossbones"
         )
```
...
valueBox(value = prettyNum(sum(world_covid_vacc$total_vaccinationsToday, na.rm = TRUE), big.mark = ","),
         caption = paste0("Total Vaccinations, ", "", "Change: ", prettyNum(sum(world_covid_vacc$Diff, na.rm = TRUE), big.mark =
                                                                                  ",")),
         color = "fuchsia",
         icon = "fas fa-syringe")
...
leaflet(World_Coordinates) %>% 
  ...
  addCircleMarkers(radius = 5,
                   color = "red",
                   popup = ~paste0("Date: 4 November 2021 ", "", 
                                   "Country/Region: ",World_Coordinates$Country_Region, "",
                                 "Province/State: ", World_Coordinates$Province_State, "",
                                 "Confirmed Cases: ",prettyNum(ts_confirmed_global$Confirmed_today, big.mark = ",") , "",
                                 "Deaths: ", prettyNum(ts_deaths_global$Deaths_today, big.mark = ","))) %>%
...
and the data was obtained from the City of Edmonton open dataset webpage using JSON and geoJSON functionality. World data is also included and retrieved from Ritchie et al. (link below)
...
was used a learning opportunity to explore the various R packages and to hone coding practices.
...
- [World COVID Data](https://ourworldindata.org/covid-deaths)

- [World COVID Time Series Data](https://data.humdata.org/dataset/novel-coronavirus-2019-ncov-cases)
