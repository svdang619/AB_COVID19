...
author: "Steven Dang"
date: "Last updated on 5 October 2021"
output:
...
ts_confirmed_global <- read_csv("time_series_covid19_confirmed_global.csv")
ts_confirmed_global <- as.data.frame(ts_confirmed_global)
ts_confirmed_global <- ts_confirmed_global %>% select(1:4, tail(names(.),1))
colnames(ts_confirmed_global) <- c("Province_State", "Country_Region", "Lat", "Long", "Confirmed")

#-----------------------------------------------------------------------------------------------------
ts_deaths_global <- read_csv("time_series_covid19_deaths_global.csv")
ts_deaths_global <- as.data.frame(ts_deaths_global)
ts_deaths_global <- ts_confirmed_global %>% select(1:4, tail(names(.),1))
colnames(ts_deaths_global) <- c("Province_State", "Country_Region", "Lat", "Long", "Deaths")

#-----------------------------------------------------------------------------------------------------
ts_recovered_global <- read_csv("time_series_covid19_recovered_global.csv")
ts_recovered_global <- as.data.frame(ts_recovered_global)
ts_recovered_global <- ts_confirmed_global %>% select(1:4, tail(names(.),1))
colnames(ts_recovered_global) <- c("Province_State", "Country_Region", "Lat", "Long", "Deaths")




leaflet(World_Coordinates, options = leafletOptions(worldCopyJump = TRUE))%>%  addProviderTiles("Esri.NatGeoWorldMap") %>% addCircleMarkers(popup = ~paste("", "Country or Region:", Country_Region,"", "", "Province or State", "", Province_State)) %>% setView(0,0,1)
